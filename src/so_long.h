...
/*   Updated: 2025/09/12 18:04:37 by phofer           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_enemy
{
	int x;
	int y;
	int dir;     /* 0=up,1=right,2=down,3=left (optional) */
}	t_enemy;
...
typedef struct s_enemies
{
	t_enemy	*arr;
	int		count;
	long	last_ms;      /* last update time in ms */
	int		interval_ms;  /* how often to move (e.g., 220) */
}	t_enemies;
...
void	set_message_with_number(t_data *data, const char *prefix, int number, const char *suffix);
...
/* enemy API */
int  enemies_init(struct s_data *d);         /* scan map for 'X' and alloc */
void enemies_free(struct s_data *d);
int  enemies_tick(struct s_data *d);         /* hook for mlx_loop_hook     */
